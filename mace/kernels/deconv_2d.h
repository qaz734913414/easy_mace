  ...
  for (index_t b = 0; b < out_shape[0]; ++b) {
    for (index_t oc = 0; oc < out_shape[1]; ++oc) {
      for (index_t oh = 0; oh < out_shape[2]; ++oh) {
        for (index_t ow = 0; ow < out_shape[3]; ++ow) {
          index_t filter_start_y, filter_start_x;
          index_t start_x = std::max<int>(0, std::ceil(float(ow - (kernel_hw[1] - 1 - padding[1])) / strides[1]));
          index_t start_y = std::max<int>(0, std::ceil(float(oh - (kernel_hw[0] - 1 - padding[0])) / strides[0]));
          filter_start_x = start_x * strides[1] + (kernel_hw[1] - 1 - padding[1]) - ow;
          filter_start_y = start_y * strides[0] + (kernel_hw[0] - 1 - padding[0]) - oh;
          filter_start_x = kernel_hw[1] - 1 - filter_start_x;
          filter_start_y = kernel_hw[0] - 1 - filter_start_y;
          T out_value = 0;
          index_t out_pos =
